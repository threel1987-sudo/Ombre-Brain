...
| Memory Edge | 自动生成显式记忆关系边，Gateway 和 `breath()` 可沿边做多跳联想浮现 | `memory_edges.py`、`memory_diffusion.py`、`reflection_engine.py` |
...
无 query：可能返回 === 核心准则 === / === 长期锚点 === / === 浮现记忆 === / === 联想浮现 ===。
  ...
  - 联想浮现：从本次命中的 anchor/普通记忆沿 memory_edges 多跳扩散，只返回短摘要和背景提醒；这是背景联想，不代表当前事实。
  - 空 query 浮现不 touch，不刷新 last_active。
有 query：返回 === 直接命中记忆 === 和可选的 === 联想浮现 ===；直接命中默认只展示最相关的 1 条，含脱水摘要和 [bucket_id:...]，扩散仍可用前几条命中作为种子。关键词和 embedding 双通道合并，默认过滤 feel。
...
3. 联想浮现来自 memory_edges 的显式关系边；embedding 相似边主要用于检索和图谱，不等于手写关系。
```
...
- Python 直跑用户可以从 `scripts/one_click.sh` 的“向量库相关”菜单执行补向量、重建向量和清孤儿向量，不需要 Docker Compose。
